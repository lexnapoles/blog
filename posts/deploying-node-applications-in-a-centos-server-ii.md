...
In the [first part of this journey](https://alejandronapoles.com/post/deploying-node-applications-in-a-centos-server-i) of configuring a Centos Server to deploy the blog and other Node applications, we did all the things we needed to do to have the server up and running in a secure way. Among other things, we configured the firewall, SSH and we modified the sshd\_config to disable password authentication, we set up Nginx with the h5bp config as a template for our own configuration. The last things we need to do are:
